...
branchlist = [f"__{e1}__{e2}" for e1, e2 in itertools.product(["PromptTau","NonPromptTau"], ["PromptLepton","NonPromptLepton"]) if not (e1 == "PromptTau" and e2 == "PromptLepton")]
...
for region in l_regions :
    ...
    if not divfake : SampleDic_NP["Nonprompt"] = ["Nonprompt(#tau_{h},"+lep+")",TColor.GetColor("#0B2447")]
    else : 
        SampleDic_NP["__NonPromptTau__PromptLepton"] = ["NP #tau_{h}",TColor.GetColor("#19376D")]
        SampleDic_NP["__PromptTau__NonPromptLepton"] = [f"NP {lep}",TColor.GetColor("#576CBC")]
        SampleDic_NP["__NonPromptTau__NonPromptLepton"] = ["NP (Both)",TColor.GetColor("#A5D7E8")]
    ...
    for (vJ,vEl,vMu) in IDcomb :
        ...
        for var in VarDic : 
            ...
            for samplename in SampleDic :
                ...
                h = f.Get(f"WRTau_SignalSingleTauTrg__PromptTau__PromptLepton/{TauID}/{var}")
                if h == None : continue
