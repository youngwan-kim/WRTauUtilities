...
for r in d_region :
    for era in l_era : 
        os.system(f"mkdir -p Outputs/{inputTag}/{d_region[r]}/{era}")
            ...
            for i,region in enumerate([f"Resolved{r}",f"Boosted{r}"]) :
                for j,channel in enumerate(["ElTau","MuTau"]) :  
                    if check(f_prompt, f"{syst}/__PromptTau__PromptLepton/{region}_{channel}/ProperMeffWR") :
                        h_data_tmp   = f_data.Get(f"{syst}/__PromptTau__PromptLepton/{region}_{channel}/ProperMeffWR")
                        h_data_tmp.SetDirectory(0)
                        l_data.append(h_data_tmp)

            for i,h in enumerate(l_data) : 
                if i == 0 :
                    h_data_real = h.Clone(f"{r}data")
                    h_data_real.SetDirectory(0)
                else : h_data_real.Add(h)
        ...
        for mwr in d_mass :
            for mn in d_mass[mwr] :
                ...
                outfile = TFile.Open(f"Outputs/{inputTag}/{d_region[r]}/{era}/WR{mwr}_N{mn}_card_input.root","RECREATE")
