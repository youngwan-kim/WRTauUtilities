...
def HADDnGet(analyzername,era,flag,outdir,skim,onlysignals) :
    ...
    os.system(f"mkdir -p ../RootFiles/{outdir}/{era}/RunXsecSyst/")
    os.system(f"mkdir -p ../RootFiles/{outdir}/{era}/RunXsecSyst/Signal")
    ...
    for mwr in signals :
        for mn in signals[mwr] :
            os.system(f"cp {hadddir}/RunXsecSyst__/{basename}_WRtoTauNtoTauTauJets_WR{mwr}_N{mn}.root ../RootFiles/{outdir}/{era}/Signals/{basename}_WRtoTauNtoTauTauJets_WR{mwr}_N{mn}.root")


    #Data
    #os.system(f"hadd ../RootFiles/{outdir}/{era}/RunXsecSyst/DATA/{basename}_DATA.root {GetSKOutDir(basename,era)}/RunXsecSyst__/DATA/{analyzername}_Tau*")
    # Prompt HADD
    for sample in samplegroup :
        if len(samplegroup[sample]) > 0 :
            ...
            for name in samplegroup[sample] :
                haddstr_AR += f"{hadddir}/RunXsecSyst__/{analyzername}_{name}.root " 
            os.system(f"hadd ../RootFiles/{outdir}/{era}/RunXsecSyst/{basename}_{sample}.root {haddstr_AR}")

        else : 
            os.system(f"hadd ../RootFiles/{outdir}/{era}/RunXsecSyst/{basename}_{sample}.root {hadddir}/RunXsecSyst__/{analyzername}_{sample}*Tau*")


    #os.system(f"hadd ../RootFiles/{outdir}/{era}/RunXsecSyst/{basename}_Prompt.root ../RootFiles/{outdir}/{era}/RunXsecSyst/{basename}_*.root ")
    os.system(f"hadd ../RootFiles/{outdir}/{era}/RunXsecSyst/{basename}_Boson_noVJets.root ../RootFiles/{outdir}/{era}/RunXsecSyst/{basename}_VVV.root ../RootFiles/{outdir}/{era}/RunXsecSyst/{basename}_VV.root")
    os.system(f"hadd ../RootFiles/{outdir}/{era}/RunXsecSyst/{basename}_Top.root ../RootFiles/{outdir}/{era}/RunXsecSyst/{basename}_TT.root ../RootFiles/{outdir}/{era}/RunXsecSyst/{basename}_ST.root")
    #os.system(f"hadd ../RootFiles/{outdir}/{era}/{basename}_Fakes.root ../RootFiles/{outdir}/{era}/RunXsecSyst/{basename}_Boson.root  ../RootFiles/{outdir}/{era}/RunXsecSyst/{basename}_TT.root ../RootFiles/{outdir}/{era}/RunXsecSyst/{basename}_ST.root ")
