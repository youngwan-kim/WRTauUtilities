...
def HADDnGet(analyzername,era,flag,outdir,skim,onlysignals) :
    ...
    #Data
    #os.system(f"hadd ../RootFiles/{outdir}/{era}/RunSyst/DATA/{basename}_DATA.root {GetSKOutDir(basename,era)}/RunSyst__/DATA/{analyzername}_Tau*")
    # Prompt HADD
    for sample in samplegroup :
        if len(samplegroup[sample]) > 0 :
            ...
            os.system(f"hadd ../RootFiles/{outdir}/{era}/RunSyst/{basename}_{sample}.root {haddstr_AR}")

        else : 
            os.system(f"hadd ../RootFiles/{outdir}/{era}/RunSyst/{basename}_{sample}.root {hadddir}/RunSyst__/{analyzername}_{sample}*Tau*")


    #os.system(f"hadd ../RootFiles/{outdir}/{era}/RunSyst/{basename}_Prompt.root ../RootFiles/{outdir}/{era}/RunSyst/{basename}_*.root ")
    os.system(f"hadd ../RootFiles/{outdir}/{era}/RunSyst/{basename}_Boson_noVJets.root ../RootFiles/{outdir}/{era}/RunSyst/{basename}_VVV.root ../RootFiles/{outdir}/{era}/RunSyst/{basename}_VV.root")
    os.system(f"hadd ../RootFiles/{outdir}/{era}/RunSyst/{basename}_Top.root ../RootFiles/{outdir}/{era}/RunSyst/{basename}_TT.root ../RootFiles/{outdir}/{era}/RunSyst/{basename}_ST.root")
    #os.system(f"hadd ../RootFiles/{outdir}/{era}/{basename}_Fakes.root ../RootFiles/{outdir}/{era}/RunSyst/{basename}_Boson.root  ../RootFiles/{outdir}/{era}/RunSyst/{basename}_TT.root ../RootFiles/{outdir}/{era}/RunSyst/{basename}_ST.root ")
