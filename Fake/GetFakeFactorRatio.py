...
d_bins_era = {
    ...
    "2016preVFP" : {   
        ...
        "Tauh_pT_11"      : [[190,250,300,350,400,450,500,600],[190,250,300,350,400,450,500,600],"p_{T}^{#tau_{h}} [GeV]"],
    ...
    "2017" : {   
        "Tauh_pT_0"      : [[190,220,250,350,450,600,1000],[190,220,250,350,450,600,1000],"p_{T}^{#tau_{h}} [GeV]"],
        "Tauh_pT_1"      : [[190,250,350,450,600,1000],[190,250,350,450,600,1000],"p_{T}^{#tau_{h}} [GeV]"],
        ...
        "ProperMeffWR_0" : [[900,1000,1100,1500,2000,4000],[900,1000,1100,1500,2000,4000],"m_{Eff} [GeV]"],
        "ProperMeffWR_1" : [[900,1000,1100,1200,1500,1800,2100,2500,4000],[900,1000,1100,1200,1500,1800,2100,2500,4000],"m_{Eff} [GeV]"],
        "ProperMeffWR_10" : [[900,1000,1100,1200,1500,1800,2100,2500,4000],[900,1000,1100,1200,1500,1800,2100,2500,4000],"m_{Eff} [GeV]"],
        "ProperMeffWR_11" : [[900,1000,1100,1200,1500,1800,2100,2500,4000],[900,1000,1100,1200,1500,1800,2100,2500,4000],"m_{Eff} [GeV]"]
    },
    ...
    "2018" : {   
        ...
        "ProperMeffWR_1" : [[900,1000,1100,1200,1500,1800,2100,2500,4000],[900,1000,1100,1200,1500,1800,2100,2500,4000],"m_{Eff} [GeV]"],
        "ProperMeffWR_10" : [[900,1000,1100,1200,1500,1800,2100,2500,4000],[900,1000,1100,1200,1500,1800,2100,2500,4000],"m_{Eff} [GeV]"],
        "ProperMeffWR_11" : [[900 ,1000,1200,1400,2000 ,2600,4000],[900 ,1000,1200,1400,2000 ,2600,4000],"m_{Eff} [GeV]"]
    },
}
...
for era in l_era :
    ...
    for var in ["Tauh_pT","ProperMeffWR"] :
        for DM in [0,1,10,11] :
            ...
            if check(f_prompt,f"Central/__PromptTau/ResolvedSignalRegion_DM{DM}/{var}") :
                h_prompt_resolved = f_prompt.Get(f"Central/__PromptTau/ResolvedSignalRegion_DM{DM}/{var}").Rebin(len(pt_bins_)-1,"prompt_res",pt_bins)
            else : h_prompt_resolved = TH1D("","",len(pt_bins_)-1,pt_bins)
            if check(f_prompt,f"Central/__PromptTau/BoostedSignalRegion_DM{DM}/{var}") :
                h_prompt_boosted  = f_prompt.Get(f"Central/__PromptTau/BoostedSignalRegion_DM{DM}/{var}").Rebin(len(pt_bins_b_)-1,"prompt_res",pt_bins_b)
            else : h_prompt_boosted = TH1D("","",len(pt_bins_b_)-1,pt_bins_b)

            h_prompt = h_prompt_boosted + h_prompt_resolved

            if check(f_ttbar,f"Central/__NonPromptTau/ResolvedSignalRegion_DM{DM}/{var}") :
                h_ttbar_resolved = f_ttbar.Get(f"Central/__NonPromptTau/ResolvedSignalRegion_DM{DM}/{var}").Rebin(len(pt_bins_)-1,"prompt_res",pt_bins)
            else : h_ttbar_resolved = TH1D("","",len(pt_bins_)-1,pt_bins)
            if check(f_ttbar,f"Central/__NonPromptTau/BoostedSignalRegion_DM{DM}/{var}") :
                h_ttbar_boosted  = f_ttbar.Get(f"Central/__NonPromptTau/BoostedSignalRegion_DM{DM}/{var}").Rebin(len(pt_bins_b_)-1,"prompt_res",pt_bins_b)
            else : h_ttbar_boosted = TH1D("","",len(pt_bins_b_)-1,pt_bins_b)

            h_ttbar = h_ttbar_resolved + h_ttbar_boosted

            if check(f_st,f"Central/__NonPromptTau/ResolvedSignalRegion_DM{DM}/{var}") :
                h_st_resolved = f_st.Get(f"Central/__NonPromptTau/ResolvedSignalRegion_DM{DM}/{var}").Rebin(len(pt_bins_)-1,"prompt_res",pt_bins)
            else : h_st_resolved = TH1D("","",len(pt_bins_)-1,pt_bins)
            if check(f_st,f"Central/__NonPromptTau/BoostedSignalRegion_DM{DM}/{var}") :
                h_st_boosted  = f_st.Get(f"Central/__NonPromptTau/BoostedSignalRegion_DM{DM}/{var}").Rebin(len(pt_bins_b_)-1,"prompt_res",pt_bins_b)
            else : h_st_boosted = TH1D("","",len(pt_bins_b_)-1,pt_bins_b)

            h_st = h_st_resolved + h_st_boosted

            if check(f_data,f"Central/ResolvedSignalRegion_DM{DM}/{var}") :
                h_data_resolved = f_data.Get(f"Central/ResolvedSignalRegion_DM{DM}/{var}").Rebin(len(pt_bins_)-1,"prompt_res",pt_bins)
            else : h_data_resolved = TH1D("","",len(pt_bins_)-1,pt_bins)
            if check(f_data,f"Central/BoostedSignalRegion_DM{DM}/{var}") :
                h_data_boosted  = f_data.Get(f"Central/BoostedSignalRegion_DM{DM}/{var}").Rebin(len(pt_bins_b_)-1,"prompt_res",pt_bins_b)
            else : h_data_boosted = TH1D("","",len(pt_bins_b_)-1,pt_bins_b)

            h_data = h_data_resolved + h_data_boosted

            h_fTT_r = TH1D(f"Resolved_DM{DM}_TT_{var}","",len(pt_bins_)-1,pt_bins)
            h_fTT_b = TH1D(f"Boosted_DM{DM}_TT_{var}","",len(pt_bins_b_)-1,pt_bins_b)
            h_fTT = TH1D(f"Inclusive_DM{DM}_TT_{var}","",len(pt_bins_)-1,pt_bins)
            h_fQCD_r = TH1D(f"Resolved_DM{DM}_QCD_{var}","",len(pt_bins_)-1,pt_bins)
            h_fQCD_b = TH1D(f"Boosted_DM{DM}_QCD_{var}","",len(pt_bins_b_)-1,pt_bins_b)
            h_fQCD = TH1D(f"Inclusive_DM{DM}_QCD_{var}","",len(pt_bins_)-1,pt_bins)
            ...
            c   = TCanvas(f"{era}_inclusive_DM{DM}_{var}",f"{era}_inclusive_DM{DM}_{var}",1000,1000)
            c_r = TCanvas(f"{era}_resolved_DM{DM}_{var}",f"{era}_resolved_DM{DM}_{var}",1000,1000)
            c_b = TCanvas(f"{era}_boosted_DM{DM}_{var}",f"{era}_boosted_DM{DM}_{var}",1000,1000)
            ...
            l.Draw()
            c.SaveAs(f"FakeFactorRatio/{stamp}/{era}_Inclusive_DM{DM}_{var}.pdf")
            c.SaveAs(f"FakeFactorRatio/{stamp}/{era}_Inclusive_DM{DM}_{var}.png")
