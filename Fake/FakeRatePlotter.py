...
def drawTagLatex(eta,nj,x=.575,y=.8) :
    latex = TLatex()
    latex.SetNDC()
    textSize = 0.625*gStyle.GetPadTopMargin()
        latex.SetTextSize(0.45*textSize)
    latex.DrawLatex(x+0.145, y1-0.095,"(#slash{E}_{T} < 100 GeV)")
...
for eta in d_geoTag :
    for nj in d_njtag :
        ...
        c = TCanvas("","",1000,1000)
        h_loose = f_fake.Get(f"WRTauFake/{r}/TauPt_Loose_{eta}_{nj}").Rebin(len(ptbins)-1,f"loose{r}",array.array('d',ptbins))
        h_tight = f_fake.Get(f"WRTauFake/{r}/TauPt_Tight_{eta}_{nj}").Rebin(len(ptbins)-1,f"tight{r}",array.array('d',ptbins))
