...
os.system(f"mkdir -p FitResults/{filestr}/PolyGauss")

'''
    f.SetParameter(0,0.1) # gaussian amplitude
    f.SetParameter(1,250)       # gaussian mean
    f.SetParameter(2,50)      # gaussian resol.
    f.SetParameter(3,0.15)      # total offset
    f.SetParameter(4,0.0001)       # exponential amplitude
    f.SetParameter(5,0.0001)        # exponential strength
    f.SetParameter(6,800)      # exponential position
'''

d_region = {#"Inclusive":[600,100],
            #"ResolvedSignalRegionMETInvert"       : [[],[],[],[450,70],[525,45]],
            #"BoostedSignalRegionMETInvert"        : [[],[],[],[450,70],[550,100]],
            #"ResolvedSignalRegionMETInvert_ElTau" : [[],[],[0.1],[0.1],[0.1,350,50,0.25,0.0001,800]],
            #"BoostedSignalRegionMETInvert_ElTau"  : [[],[],[0.1,250,50,0.15,0.0001,300],[0.1],[0.1]],
            "ResolvedSignalRegionMETInvert_MuTau" : [[],[],[],[0.1,300,20,0.00005,200],[0.07,300,40,1e-4,200.]],
            #"BoostedSignalRegionMETInvert_MuTau"  : [[],[],[0.1,250,50,0.15,0.001,400],[0.1],[0.1,250,50,0.15,0.0001,600]],
}
...
output_file_test = TFile(f"FitResults/{filestr}/PolyGauss/FitResults.root", "RECREATE")
...
for j, era in enumerate(["2016preVFP","2016postVFP","2016","2017","2018"]):
    for r in d_region :
        if len(d_region[r][j]) == 0 : continue
            ...
            polynomial_formula = "[0] * exp(-(x-[1])**2/(2*[2]**2)) +  [3] * (x-[4])**2 + [5]"
            ...
            if len(d_region[r][j]) == 5 :
                ...
                f.SetParameter(2,50)      # gaussian resol.
                f.SetParameter(3,0.15)      # total offset
                f.SetParameter(4,0.0001)        # exponential strength
                #f.SetParameter(5,0)      # exponential position
            
            f.SetParameter(5,0.02)
            f.FixParameter(0,0.075)
            f.FixParameter(5,0.035)
            ...
            l = TLegend(0.575,0.6,0.865,0.775)
            l.SetFillStyle(0)
            l.SetBorderSize(0)
            canvas.cd()
            ...
            latex.DrawLatex(0.525,0.825,"FR(x) = A_{0}N(#mu,#sigma^{2}) + A_{1} (x-A_{2})^{2} ")
            #latex.DrawLatex(0.65,0.785,"+ A_{1} (x-A_{3})^{2} ")
            f.SetLineColor(kRed)
            f.SetFillStyle(0)
            f.SetLineWidth(3)
            f.Draw("same")
            ...
            l.AddEntry(f_label,"#splitline{Gaussian+Quadratic}{#scale[0.75]{( #chi^{2}/ndf = "+pol_fitness+" )}}","l")
            ...
            #output_file = TFile(f"FitResults/{filestr}/PolyGauss/FitFunction_{r}_{era}_PolyGauss.root", "RECREATE")
            ...
            canvas.Update()
            canvas.Draw()
            canvas.SaveAs(f"FitResults/{filestr}/PolyGauss/FitPlot_{r}_{era}_PolyGauss.png")
            canvas.SaveAs(f"FitResults/{filestr}/PolyGauss/FitPlot_{r}_{era}_PolyGauss.pdf")
            canvas.Close()
