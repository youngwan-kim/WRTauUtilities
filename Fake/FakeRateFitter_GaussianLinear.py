...
os.system(f"mkdir -p FitResults/{filestr}/GaussainLinearTransition")

'''
f.SetParameter(0,boundary) # transition pT
f.SetParameter(1,10)   # transition steepness
f.SetParameter(2,0.1) # gaussian amplitude 
f.SetParameter(3,200) # gaussian mean
f.SetParameter(4,20)  # gaussian resol.
f.SetParameter(5,0)   # linear slope
f.SetParameter(6,0.2) # linear interscept
...
d_region = {#"Inclusive":[600,100],
            ...
            "ResolvedSignalRegionMETInvert_ElTau" : [[],[],[500,100,0.125,225,15,0.001,0.01,0.2],[350,20,0.1,200,20,0,0.2,0.5],[]],
            "BoostedSignalRegionMETInvert_ElTau"  : [[],[],[320,20,0.1,250,20,0,0.2,0.15],[380,19.75,0.1,300,17.5,0,-0.001,0.15],[400,30,0.1,300,20,0,0.001,0.15]],
            "ResolvedSignalRegionMETInvert_MuTau" : [[],[],[550,20,0.1,200,20,0.,0.2,0.5],[],[]],
            "BoostedSignalRegionMETInvert_MuTau"  : [[],[],[380,48,0.1,250,30,0,-0.3,0.1],[],[350,16,0.1,300,20,0,0.001,0.1]]}
...
output_file_test = TFile(f"FitResults/{filestr}/GaussainLinearTransition/FitResults.root", "RECREATE")
...
for j, era in enumerate(["2016preVFP","2016postVFP","2016","2017","2018"]):
    for r in d_region :
        if len(d_region[r][j]) == 0 : continue
            ...
            polynomial_formula = "( [0] + [1]*x + [2]*x**2 + [3]*x**3 ) * exp([5]*x)"

            polynomial_formula = "[0]/( 1 + exp( -( x-[2] )/[1] ) ) + [3] * exp( -[4]*(x-[5])**2 )"

            polynomial_formula = "(1 - 1/(1+exp(-(x-[0])/[1]))) * [2] * ( exp(-(x-[3])**2/(2*[4]**2)) + [7] ) + 1/(1+exp(-(x-[0])/[1])) * ([5]*x + [6])"
            ...
            if len(d_region[r][j]) == 8 :
                for i in range(0,8) :
                    f.SetParameter(i,d_region[r][j][i])

            else :
                f.SetParameter(0,boundary) # transition pT
                f.SetParameter(1,20)   # transition steepness
                f.SetParameter(2,0.1) # gaussian amplitude 
                f.SetParameter(3,200) # gaussian mean
                f.SetParameter(4,20)  # gaussian resol.
                f.SetParameter(5,0)   # linear slope
            ...
            l = TLegend(0.575,0.6,0.865,0.75)
            l.SetFillStyle(0)
            l.SetBorderSize(0)
            canvas.cd()
            ...
            latex.DrawLatex(0.55,0.825,"FR(x) = {1-#sigma(x*,#alpha)} #upoint N(#mu,#sigma^{2})")
            latex.DrawLatex(0.65,0.785,"+ #sigma(x*,#alpha) #upoint {mx+b}")
            f.SetLineColor(kRed)
            f.SetFillStyle(0)
            f.SetLineWidth(3)
            f.Draw("same")
            ...
            #output_file = TFile(f"FitResults/{filestr}/GaussainLinearTransition/FitFunction_{r}_{era}_GaussianLinear.root", "RECREATE")
            ...
            canvas.Update()
            canvas.Draw()
            canvas.SaveAs(f"FitResults/{filestr}/GaussainLinearTransition/FitPlot_{r}_{era}_GaussianLinear.png")
            canvas.SaveAs(f"FitResults/{filestr}/GaussainLinearTransition/FitPlot_{r}_{era}_GaussianLinear.pdf")
            canvas.Close()
