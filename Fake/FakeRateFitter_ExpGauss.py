...
os.system(f"mkdir -p FitResults/{filestr}/ExpGauss")
...
d_region = {#"Inclusive":[600,100],
            ...
            "ResolvedSignalRegionMETInvert_MuTau" : [[],[],[0.1,250,50,0.15,0.001,400],[0.1,300,75,0,0,0,0],[0.1,300,20,0.,0.,0,0]],
...
for j, era in enumerate(["2016preVFP","2016postVFP","2016","2017","2018"]):
    for r in d_region :
        if len(d_region[r][j]) == 0 : continue
            ...
            polynomial_formula = "[0] * exp(-(x-[1])**2/(2*[2]**2))  +  exp( [4]*(x-[5] )) + [3] + [6]*x**2"
            ...
            if len(d_region[r][j]) == 7 :
                ...
                f.SetParameter(5,800)      # exponential position
                f.SetParameter(6,0)      #  position
            ...
            output_file = TFile(f"FitResults/{filestr}/ExpGauss/FitFunction_{r}_{era}_ExpGauss.root", "RECREATE")
            f.Write()
            hint0.Write()
            #g.Write()
            ...
            canvas.Update()
            canvas.Draw()
            canvas.SaveAs(f"FitResults/{filestr}/ExpGauss/FitPlot_{r}_{era}_ExpGauss.png")
            canvas.SaveAs(f"FitResults/{filestr}/ExpGauss/FitPlot_{r}_{era}_ExpGauss.pdf")
            canvas.Close()